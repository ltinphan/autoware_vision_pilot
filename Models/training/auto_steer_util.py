...
def visualize(samples, output_xp, output_h_vector, target_xp, target_h_vector):
    first_idx = 0 # first element in the batch
    img = samples[first_idx].squeeze(-1).cpu().detach().numpy()
    # Change from CHW -> HWC
    img_np = np.transpose(img, (1, 2, 0))  # shape now (512, 1024, 3)
    ...
    e_xp = output_xp[first_idx][0].squeeze(-1).cpu().detach().numpy() * W  # shape (64,)
    e_h_vector = output_h_vector[first_idx][0].squeeze(-1).cpu().detach().numpy()
